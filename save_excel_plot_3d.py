"""this is the code which takes in 2 videos and then uses our model and fnds 3D co-ordinates. Bounding box is made using YOLOv7 
and then this box is used to calc the center hich is used as output, output is displayed in terminal.
output is saved as excel file in the directory by the name     fish_3D_coords.xlsx      """
...
# List to store fish coordinates with frame number
fish_3D_coords_list = []
...
while True:
    ...
    for box in results1[0]:
        ...
        cy = int((y1 + y2) / 2)
        fish_3D_coords.append([frame_number, *points_3D[cy, cx]])  # Add frame number and get the 3D coordinates
...
df = pd.DataFrame(fish_3D_coords_list, columns=['Frame', 'X', 'Y', 'Z'])
...
fish_3D_coords_array = np.array(fish_3D_coords_list)
ax.scatter(fish_3D_coords_array[:, 1], fish_3D_coords_array[:, 2], fish_3D_coords_array[:, 3])  # X, Y, Z columns

ax.set_xlabel('X Label')
ax.set_ylabel('Y Label')
ax.set_zlabel('Z Label')

# trying to plot it but failing super well yayyyyyyy :)
# it's a backend error so im trying out colab and that is super stupid and ijust wanna murder someone right now
